...
class ConvTranspose(nn.Module):
    ...
    def forward(self, x):
        y = self.act(self.bn(self.conv_transpose(x)))
        if not self.training:
            self.features = y.detach()
        return y
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.tr(p + self.linear(p)).permute(1, 2, 0).reshape(b, self.c2, w, h)
        if not self.training:
            self.features = y.detach()
        return y
...
class BottleneckCSP(nn.Module):
    ...
    def forward(self, x):
        ...
        y2 = self.cv2(x)
        y = self.cv4(self.act(self.bn(torch.cat((y1, y2), 1))))
        if not self.training:
            self.features = y.detach()
        return y
...
class C2Base(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.cv2(torch.cat((self.m(a), b), 1))
        if not self.training:
            self.features = y.detach()
        return y
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        y = x * self.act(self.fc(self.pool(x)))
        if not self.training:
            self.features = y.detach()
        return y
...
class ChannelAttention2(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        y = x * self.act(self.cv1(self.pool1(x) + self.pool2(x)))
        if not self.training:
            self.features = y.detach()
        return y
...
class ChannelAttention3(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        y = x * self.act(self.cv2(self.silu(self.cv1(self.pool1(x)))) + self.cv2(self.silu(self.cv1(self.pool2(x)))))
        if not self.training:
            self.features = y.detach()
        return y
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        y = x * self.act(self.cv1(torch.cat([torch.mean(x, 1, keepdim=True), torch.max(x, 1, keepdim=True)[0]], 1)))
        if not self.training:
            self.features = y.detach()
        return y
...
class Contract(nn.Module):
    ...
    def forward(self, x):
        ...
        y = x.view(b, c * s * s, h // s, w // s)  # x(1,256,40,40)
        if not self.training:
            self.features = y.detach()
        return y
...
class Expand(nn.Module):
    ...
    def forward(self, x):
        ...
        y = x.view(b, c // s ** 2, h * s, w * s)  # x(1,16,160,160)
        if not self.training:
            self.features = y.detach()
        return y
...
class Proto(nn.Module):
    ...
    def forward(self, x):
        y = self.cv3(self.cv2(self.upsample(self.cv1(x))))
        if not self.training:
            self.features = y.detach()
        return y
...
class Classify(nn.Module):
    ...
    def forward(self, x):
        if isinstance(x, list):
            x = torch.cat(x, 1)
        y = self.linear(self.drop(self.pool(self.conv(x)).flatten(1)))
        if not self.training:
            self.features = y.detach()
        return y
