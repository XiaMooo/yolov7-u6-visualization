...
def plot_multi_images(*images, grid=0, title=None, dpi=60):
    n = len(images)
    assert n > 1, "Func needs at least 2 image as param!"
    row_colum = 100 + n * 10
    plt.figure(figsize=(5 * n, 8), dpi=dpi)
    if title is not None:
        plt.title(title)
...
def plot_multi_heatmaps(*arrays, cmap='jet', title=None, row=None, col=None, dpi=60):
    n = len(arrays)
    if row is not None:
    ...
    fig, ax = plt.subplots(row, col, figsize=(6 * col, 4 * row), dpi=dpi)
